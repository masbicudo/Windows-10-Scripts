...
// WScript.StdErr.WriteLine(JSON.stringify(pkgrgx.source))
// WScript.StdErr.WriteLine(JSON.stringify(idrgx.source))
...
for (var p = 0; p < json.length; p++) {
    ...
    if (pkgrgx.test(pkg.Name)) {
        ...
        for (var j = 0; j < nodes.length; j++) {
            ...
            if (idrgx.test(id)) {
                WScript.StdOut.WriteLine(pkg.PackageFamilyName+"!"+id)
