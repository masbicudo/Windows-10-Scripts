...
var arg_spec = {
    ...
    "r": "replace", "replace": 1,
    "s": "select", "select": 1,
    "sl": "select-line", "select-line": 0,
    "d": "debug", "debug": 0,
    "i": "case-insensitive", "case-insensitive": 0,
    "c": "command", "command": -1,
    "t": "text", "text": 1,
    "sl": "string-literal", "string-literal": 1,
    "js": "javascript", "javascript": 1,
    "jsfn": "javascript-function", "javascript-function": 1
}
var args = []
var cnt = 0
var name = null
...
function linesStream(lines, fnProcArgs) {
    lines = lines instanceof Array ? lines : [lines]
    lines = map.call(lines, function(x){return fnProcArgs(x)})
    lines = map.call(lines, function(x){return x.split(/\r\n|\n/g)})
    lines = [].concat.apply([], lines)
    WScript.Echo(JSON.stringify(lines))
    s = {
        AtEndOfStream: lines.length == 0,
        ReadLine: function() {
            this.AtEndOfStream = lines.length <= 1
            return lines.shift()
        },
        Close: function(){}
    }
    //WScript.Echo(JSON.stringify(s))
    return s
}

if ("text" in args) {
    s = linesStream(args.text, function(x){return x})
}
if ("string-literal" in args) {
    s = linesStream(args["string-literal"], function(x){return eval('"'+x+'"')})
}
if ("javascript" in args) {
    s = linesStream(args.javascript, function(x){return eval(x)})
}
if ("javascript-function" in args) {
    s = linesStream(args["javascript-function"], function(x){return Function(x)()})
}
...
//WScript.Echo(JSON.stringify(args))
...
while (!s.AtEndOfStream) {
    ...
    if (args["select-line"]) {
        ...
        // WScript.Echo(JSON.stringify(map.call(rs, function(x){return x.toString()})))
        // WScript.Echo(JSON.stringify(execs))
        if (every.call(execs)) {
