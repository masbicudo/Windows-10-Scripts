...
// encoding methods
// - "s"/"set": encode the input to be set to a variable like this: for /f "tokens=*" %%a IN (' xpto | enc "set" ') do set _var=%%a
// - "c"/"call": encode the input to be used in a call
// - "q"/"forbq": encode the input to be used inside a for with usebackq param
// - "f"/"for": encode the input to be used inside a for without usebackq param
// - "a"/"arg": encode the input to be used as a parameter to a command
// - "b"/"batarg": encode the input to be used as a parameter to a batch script
// - "t"/"bat": encode the input to be used in the context of a batch file
// encoding modes
// - "+d"/"+delayed": enable delayed to encode the input to be used inside a delayed expansion context
// - "-d"/"-delayed": disable delayed to encode the input to be used inside a delayed expansion context
// many options at once are allowed:
// - "+d set for arg": applies enc_arg, enc_delayed, enc_for, enc_set
// - "+d s f c t" applies enc_batarg, enc_
